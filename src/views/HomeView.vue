...
const getIngredients = (
  data: SoilSisters[]
): { links: Link[] | undefined; nodes: Node[] | undefined } => {
  ...
  const allIngredients =
    ingredients?.children
      ?.map((d) =>
        (d as Supply)["Regenerative Farming Group"]?.split(";").map((i) => i.trim())
      )
      .flat() ?? [];

  const links = outputs?.children?.map((d) => {
    const ingredientNames = allIngredients.filter(Boolean);
    const ingredientIndices = ingredientNames
      .map((d) => ingredientsArray?.indexOf(d ?? ""))
  ...
  const counts = allIngredients.reduce((acc, curr) => {
  ...
  const nodes = combinedArray.map((d) => ({
    ...
    fao: parseInt(
      ingredientsArray.includes(d)
        ? (ingredients?.children?.find(
            (i) => (i as Supply)["Entry Name"] === d
          ) as Supply)?.FAO
        : (outputs?.children?.find((i) => i["Entry Name"] === d) as Supply)?.FAO
    ),
    type: ingredientsArray.includes(d) ? "ingredient" : "output",
    data: ingredientsArray.includes(d)
      ? ingredients?.children?.find((i) => (i as Supply)["Entry Name"] === d)
      : outputs?.children?.find((i) => i["Entry Name"] === d),
  }));
...
const isIngredients = (data: SoilSisters[]): Supply[] => {
  return data
    .filter((d) => d.sheetName === "Supply")
    .flatMap((d) => d.children)
    .sort((a, b) => a["Entry Name"]?.localeCompare(b["Entry Name"])) as Supply[];
};

const isOutputs = (data: SoilSisters[]): Demand[] => {
  return data
    .filter((d) => d.sheetName === "Demand")
    .flatMap((d) => d.children)
    .sort((a, b) => a["Entry Name"]?.localeCompare(b["Entry Name"])) as Demand[];
};
...
const handleSideBarEmitted = (item: Supply | Demand, type: string) => {
  ...
  const dispatchEventToNodes = (eventType: "mouseover" | "mouseout") => {
    const event = new MouseEvent(eventType);
    resetZoom();
    nodeGroupsRef.value?.nodes().forEach((node: Element) => {
      node.dispatchEvent(new MouseEvent("mouseout"));
    });

    if (item !== null) {
      const specificNode = nodeGroupsRef.value
        ?.filter((d: Node) => d.name === item["Entry Name"])
        .node();
      specificNode?.dispatchEvent(event);
      if (specificNode) zoomToNode(specificNode);
    } else {
      nodeGroupsRef.value?.nodes().forEach((node: Element) => {
...
<template>
  <main>
    ...
    <SideBar
      pos="left"
      :sheetName="store.originalData[0].sheetName"
      :children="isIngredients(store.data)"
      @update:selectedItem="handleSideBarEmitted"
    />
    <SideBar
      pos="right"
      :sheetName="store.originalData[1].sheetName"
      :children="isOutputs(store.data)"
      @update:selectedItem="handleSideBarEmitted"
    />
    <div class="wrapper">
